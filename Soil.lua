...
function buildSoil(world,door)
    ...
    for ysoil = 53, 0, -1 do 
        ...
        for xsoil = soil1, soil2, soil3 do
            if (not multyBot or indexBot == 1) and worldWater ~= "" then 
                for _,tile in pairs(bot:getWorld():getTiles()) do 
                    while bot:getInventory():findItem(822) == 0 do 
                        takeWater(world,door)
                        sleep(100)
                    end
                    if tile.x == 0 and tile.y <= 53 and getTile(tile.x,tile.y).fg == 0 and tile.y > 0 then 
                        bot:findPath(xsoil,ysoil)
                ...
                for _,tile in pairs(bot:getWorld():getTiles()) do 
                    ...
                    if tile.x == 99 and tile.y <= 53 and getTile(tile.x,tile.y).fg == 0 and tile.y > 0 then 
                        bot:findPath(xsoil,ysoil)
                        while not bot:getWorld():getTile(tile.x,tile.y):hasFlag(1024) do
                            place(822,0,0)
                            sleep(200)
...
function takeWater(worlds,doors)
    if worldWater ~= "" then 
        bot.auto_collect = false 
        sleep(100)
        warp(worldWater,doorWater)
        sleep(100)
        if not nuked then 
            while bot:getInventory():findItem(822) == 0 do 
                for _,obj in pairs(bot:getWorld():getObjects()) do 
...
while true do 
    ...
    unplaceds = 0
    if multyBot then 
        dataWorld = takeWorld(listWorld)[1]
    else 
        dataWorld = takeWorld(listWorld)[indexBot]
    end
    if dataWorld then
    world = dataWorld.world
    door = dataWorld.door
    end
