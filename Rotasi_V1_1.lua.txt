...
restBotMode = "HOUR"      -- 2 mode HOURS & INTERVAL keep "" want't rest
...
webhookPack = "https://discord.com/api/webhooks/1205900838095163472/hrc47RfZ9iXgK03P2IZ7F5vg2koD3LPMyw4GR9Tln6Qjs8BjrKj1yK2rGVes8XDANbvw"  -- Webhook info Pack
...
function restingBot(world,door,x,y)
    ...
    elseif restBotMode == "INTERVAL" then 
        restNow = rb + intervalTime
        if currentTime >= restNow then 
            if disconnectOnRest then 
                if getStatus(bot) == "Online" then 
                    bot.auto_reconnect = false 
                    sleep(100)
        ...
        while bot.status ~= BotStatus.online or bot:getPing() == 0 do
            if rotatingProxy then 
                repeat
                    bot:connect()
                    sleep(delayRecon)
                    if getStatus(bot) == "Changing Subserver" and bot:getPing() ~= 0 then 
                        sleep(10000)
                    end
                until getStatus(bot) == "Banned" or getStatus(bot) == "Online"
                if getStatus(bot) == "Online" then 
...
function botEvent(status)
    ...
    if webhookEventBot ~= "" then 
        ...
        syn.embed1.footer.text = "Lucifer Rotation V.1.1 \n"..os.date("!%a %b %d, %Y at %I:%M %p", os.time() + 7 * 60 * 60)
        if msgIdEvent ~= "" then 
            syn:edit(msgIdEvent)
        else 
            syn:send()
...
function packInfo(wb,id,desc)
    if wb ~= "" then 
        ...
        syn.embed1.thumbnail = "https://cdn.discordapp.com/attachments/1180523579381665933/1180577805403181076/20231203_013427.png"
        syn.embed1:addField("**<:ubisyn:1235474439689207862> WORLD**","||"..bot:getWorld().name.."||",true)
        syn.embed1:addField("**<:bot:1229904719720484990> LAST DROP**",bot.name.." (No."..indexBot..")",true)
        syn.embed1:addField("**<:orbs:1235606674497339422> DROPPED ITEMS**",desc,true)
        syn.embed1.footer.icon_url = "https://cdn.discordapp.com/attachments/1180523579381665933/1180577805403181076/20231203_013427.png"
        syn.embed1.footer.text = "Lucifer Rotation V.1.1 by SYN\n"..os.date("!%a %b %d, %Y at %I:%M %p", os.time() + 7 * 60 * 60)
        if id ~= "" then 
            syn:edit(id)
        else 
            syn:send()
